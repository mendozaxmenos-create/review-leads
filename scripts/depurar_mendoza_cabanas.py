"""Depura el CSV/CRM de la campaña Mendoza · cabañas.

Uso:
  .venv\\Scripts\\python -m scripts.depurar_mendoza_cabanas
  .venv\\Scripts\\python -m scripts.depurar_mendoza_cabanas --csv data/exports/mendoza-cabanas-....csv
  .venv\\Scripts\\python -m scripts.depurar_mendoza_cabanas --no-crm
"""
...
def _default_csv() -> Path:
    ...
    candidates = sorted(export_dir.glob("mendoza-cabanas-*.csv"), reverse=True)
    # Prefer raw campaign exports over ready/discarded
    for path in candidates:
        name = path.name
        if "ready" in name or "discarded" in name:
            continue
        return path
    raise FileNotFoundError("No hay CSV mendoza-cabanas-*.csv en data/exports")


def main() -> int:
    parser = argparse.ArgumentParser(description="Depurar leads Mendoza cabañas")
    ...
    ready_path = export_dir / f"mendoza-cabanas-ready-{stamp}.csv"
    discarded_path = export_dir / f"mendoza-cabanas-discarded-{stamp}.csv"
    # También aliases estables
    ready_stable = export_dir / "mendoza-cabanas-ready.csv"
    discarded_stable = export_dir / "mendoza-cabanas-discarded.csv"
    ...
    if not args.no_crm:
        ...
        for row in discarded:
            ...
            store.update_saved_lead(
                info["saved_lead_id"],
                status="discarded",
                notes=f"Depurado Mendoza: {row.get('depurar_reason')}",
            )
            crm_updated += 1
